(ns emotiv.main
  "Kotodama WASM entrypoint for the Emotiv clean-room actor (L4) — Clojure port.
  ...
  facts (`emotiv.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "emotiv")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"emotiv.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "emotiv-compat" :tier tier :entities entities} 200])
...
(defn start! [] :emotiv-compat/ready)
